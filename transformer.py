...
def scaled_dot_product_attention(query, key, value):
    key_query_product = tf.nn.softmax(
        tf.einsum("bij, bij -> bi", query, key) / tf.math.sqrt(tf.cast(query.shape[-2], dtype=tf.float32)))
...
class MultiheadAttention(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        for i in range(self.d_k):
            self.attention_result[i] = scaled_dot_product_attention(self.query_projection[i], self.key_projection[i],
                                                                    self.value_projection[i])
